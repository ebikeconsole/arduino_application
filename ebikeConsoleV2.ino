...
const float deuxPi = 6.28318; // Deux Pi (Calcul de la distance)
const float Pi = 3.14159;		// Pi (Calcul de la distance)


// --- constantes des broches ---
const int wheelSensorPin = 2; // variable de broche
const int currentSensorPin = A0;	  
const int voltBatPin = A1;		   //Mesure de tenson
...
int distance;					//Distance en cours					eeprom   effaçable
int distanceTotal;			//Distance Totale (jamais effacé)	eeprom   non-effaçable
float hauteurRoue;				//Rayon de roue						eeprom   paramétrable
...
//Mesure Tension
float batVolt = 0;
float batVoltMoyen = 0;
int cptMoyenne = 0;
float batVoltMoyenAff = 0;

//Gestion Eeprom
int infoSave = 0;
 
//Gestion de la période de vitesse
...
void Timer(void)
{
	...
	delayMicroseconds(100);
	batVolt = batVolt * 11.23;					  //   R1/R2
	batVoltMoyen +=batVolt;
	++cptMoyenne;
	if(cptMoyenne>=100)
	{
		batVoltMoyenAff = batVoltMoyen/100;
		cptMoyenne=0;
		batVoltMoyen=0;
	}
...
void loop(){
	...
	Serial.print("  ");Serial.print(batVoltMoyenAff);Serial.print("Volt");						//Debug volt
	Serial.print("  ");Serial.print(freeRam ());Serial.print("RAM");					//Debug freeRAm
	Serial.print("\n");
